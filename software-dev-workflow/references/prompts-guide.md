...
## 输出格式
CTO 路由模板，`CTO路由` 或 `项目形态` 注明 `POC/Spike`，`停止条件` 必须包含升级触发条件。
...
4. 把场景信息写进 CTO 路由模板的 `CTO路由` 和 `项目形态` 字段，例如：`CTO路由: Scenario | OCR / 文档智能 | Architecture`、`Python Agent/CLI（场景：OCR / 文档智能）`。
...
## 输出格式
CTO 路由模板，`CTO路由` 与 `项目形态` 必须带场景标签；`下一步 3 个动作` 至少包含「补齐场景 design doc 必写章节」与「跑场景 checklist」各 1 条。
