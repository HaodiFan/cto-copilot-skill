...
用于 Stage 5-8：把已经确认的需求、spec 和架构，推进到可验证、可审查、可发布、可沉淀的实现。它吸收三类经验：
...
| Stage | 使用本文件的方式 |
|---|---|
| Stage 5 Feature 规划 | 用 §2 判断 checkout/worktree，用 §3 输出 Implementation Plan，用 §4 标 review roles |
| Stage 6 实现 | 用 §2 再次确认 checkout/worktree；按 slice 执行，每个 slice 完成后跑对应 validation gate |
